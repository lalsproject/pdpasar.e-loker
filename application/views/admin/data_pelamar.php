...
<br>
<?php if ($l->flag_berkas == 'N'){ ?>
	<button type="button" class="btn btn-sm btn-success" onclick="cek_berkas('<?= encrypt($l->id_lamaran) ?>','<?= encrypt('Y') ?>')">Terima</button>
	<button type="button" class="btn btn-sm btn-outline-primary" onclick="cek_berkas('<?= encrypt($l->id_lamaran) ?>','<?= encrypt('N') ?>')">Tolak</button>	
	<script>
